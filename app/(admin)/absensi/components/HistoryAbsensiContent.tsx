...
export function HistoryAbsensiContent() {
  ...
  const filteredStaff = staff.filter(s => s.nama.toLowerCase().includes(search.toLowerCase()));
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="py-10 text-center text-slate-400">Tidak ada data.</td>
                </tr>
              ) : (
                filteredStaff.map((s) => {
                  ...
                  const alpa = sAtt.filter(a => a.status === 'Alpha').length;
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-5 py-4 text-center font-bold text-blue-600">{sakit}</td>
                      <td className="px-5 py-4 text-center font-bold text-amber-600">{izin}</td>
                      <td className="px-5 py-4 text-center font-bold text-rose-600">{alpa}</td>
                      <td className="px-5 py-4 text-center">
                        <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${rate >= 80 ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}>
                          {rate}%
                        </span>
                      </td>
                    </tr>
