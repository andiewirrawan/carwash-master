...
export async function addCustomer(item: Omit<Customer, 'id' | 'total_kunjungan' | 'total_omzet' | 'tier'>): Promise<Customer> {
  const payload = {
    ...item,
    tier: 'reguler',
    total_kunjungan: 0,
...
export async function addKomisiManual(item: Omit<KomisiManual, 'id' | 'created_at'>): Promise<KomisiManual> {
...
fs.writeFileSync('lib/db.ts', code, 'utf-8');
console.log("Successfully rewrote lib/db.ts");
